...
#  modified by: Kevin King -- added get_neighbors and get_color functions for HMM use
...
class Maze:
    ...
    def get_neighbors(self, x, y):
        neighbors = []
        # Define possible moves as (dx, dy) pairs
        moves = [(0, 1), (0, -1), (0, 1), (0, -1)]
        for dx, dy in moves:
            nx, ny = x + dx, y + dy
            # Check if the new position is a floor and is within the maze boundaries.
            if 0 <= nx < self.width and 0 <= ny < self.height and self.is_floor(nx, ny):
    ...
    def update_robot_location(self, location):
        self.robotloc = [location[0], location[1]]
